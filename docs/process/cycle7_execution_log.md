...
- **[bin/install_infra.sh](../../bin/install_infra.sh)** (Cập nhật): Tự động tạo tệp tin `conf/mrjob.conf` động ngay sau khi cài đặt thành công python venv ở Bước 7.
- **[bin/run.sh](../../bin/run.sh)** (Cập nhật): Tự động cập nhật nội dung tệp tin `conf/mrjob.conf` mỗi khi ứng dụng chạy để tương thích với trường hợp thư mục dự án bị đổi tên hoặc di chuyển.
- **[src/mapreduce/run_all_jobs.py](../../src/mapreduce/run_all_jobs.py)** (Cập nhật):
...
- **[src/streamlit_app/app.py](../../src/streamlit_app/app.py)** (Cập nhật):
...
3. **Cập nhật các Script hệ thống**:
   - **[bin/setup.sh](../../bin/setup.sh)** (Cập nhật): Đổi lệnh cài đặt Java mặc định khi thiếu sang `openjdk-8-jdk` thay vì `openjdk-11-jdk`.
   - **[docs/MASTERPLAN.md](../../docs/MASTERPLAN.md)** (Cập nhật): Cập nhật mô tả Cycle 0 về Java 8 thay vì Java 11.
...
- **[bin/run.sh](../../bin/run.sh)** (Cập nhật):
...
1. **Thiết lập Master Test Plan**:
   - Cập nhật tài liệu [TEST_PLAN.md](../../TEST_PLAN.md) thành kế hoạch kiểm thử tổng thể, chi tiết hóa 7 chu kỳ kiểm thử thủ công từ Cycle 0 đến Cycle 6 và giới thiệu cách kiểm thử tự động.
...
3. **Phát triển Script chạy kiểm thử tự động**:
   - Tạo tệp shell [bin/run_tests.sh](../../bin/run_tests.sh) thiết lập môi trường Java 8, Hadoop, Hive, venv và kích hoạt bộ test tự động.
...
1. **Tự động hóa Hive Metastore & HiveServer2**:
   - Cập nhật [bin/run.sh](../../bin/run.sh): Tự động phát hiện cổng và kích hoạt dịch vụ Hive Metastore (cổng 9083) và HiveServer2 (cổng 10000) ở chế độ nền. Thêm vòng lặp đợi 30 giây để đảm bảo HiveServer2 khởi động thành công trước khi chạy Streamlit.
   - Cập nhật [bin/stop.sh](../../bin/stop.sh): Kích hoạt lệnh `pkill` để giải phóng hoàn toàn các dịch vụ Hive chạy nền khi dừng hệ thống.
2. **Nâng giới hạn bộ nhớ JVM Heap**:
   - Cập nhật [bin/run.sh](../../bin/run.sh): Kích hoạt `export HADOOP_CLIENT_OPTS="-Xmx1024m $HADOOP_CLIENT_OPTS"` trực tiếp trước khi chạy các daemon, nâng giới hạn bộ nhớ Heap lên 1GB để tránh các lỗi OutOfMemory cục bộ.
3. **Kiểm soát Mock Data và Giao diện Biểu đồ**:
   - Cập nhật [src/streamlit_app/hive_connector.py](../../src/streamlit_app/hive_connector.py): Loại bỏ hoàn toàn cơ chế tự động chuyển vùng sang mock data khi lỗi. Bổ sung tham số kiểm soát `use_mock_data` rõ ràng cho các hàm truy vấn.
   - Cập nhật [src/streamlit_app/app.py](../../src/streamlit_app/app.py):
...
4. **Cập nhật tối ưu từ người dùng**:
   - Cập nhật [bin/run.sh](../../bin/run.sh): Tích hợp kiểm tra cổng MySQL (3306) và MongoDB (27017) trước khi chạy để tránh khởi chạy trùng lặp. Tự động tạo tệp cấu hình `conf/mrjob.conf` động để liên kết môi trường ảo `venv`.
   - Cập nhật [src/streamlit_app/app.py](../../src/streamlit_app/app.py): Đồng bộ động `conf/mrjob.conf` ngay khi Streamlit tải và sử dụng `sys.executable` chạy các script con MapReduce/init_db đảm bảo tính di động cao.
